...
def scan_existing(dest: Path) -> ExistingIndex:
    ...
    Uses os.walk with onerror so the scan tolerates directories that
    appear or disappear mid-walk (e.g. when an external organizer is
    actively moving files in the dest tree).
    """
    import os
    ...
    if not dest.exists():
        return idx
    for root, dirs, files in os.walk(str(dest), onerror=lambda _e: None):
        for d in dirs:
            name = d.lower()
            idx.dirs.add(name)
